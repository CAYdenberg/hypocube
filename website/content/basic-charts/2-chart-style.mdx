...
<PropDef name="dataPointSize" type="number" def="5">
  A <strong>data point</strong> is simply a visual marker at defined
  coordinates. It is most commonly used as the markers in a LineSeries, alone or
  in conjunction with a data line. This style property refers to the size in
  pixels of the data point from one end to the other. In other words, the data
  point is drawn to fit within a square of `dataPointSize` pixels along either
  dimension.
</PropDef>
...
<PropDef name="dataLineCurveType" type="string | D3CurveFactory" def="linear">
  A <strong>data line</strong> is simply a line drawn between all of the data
  points in a series. It is most commonly used as the marker in a Line series,
  alone or in conjunction with a data point. The "curve type" represents a
  function to calculate the shape of the line in between the data points, and
...
<PropDef name="dataLineStrokeWidth" type="color" def="#000"></PropDef>
