...
<PropDef name="svgPointerEvents" type="boolean" def="true">
  ...
  This can help reduce jittering or jankiness when the desired primary point of
  interaction is on the `Chart` itself, rather than internally rendered
  elements. When `isCanvas={true}` on the `Chart` component, this property has
  no effect.
</PropDef>
...
<PropDef name="seriesXOffset" type="number" def="0">
  ...
  The <a href="./7-series-styles">`seriesStyles' addon</a> can be used to
