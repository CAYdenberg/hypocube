...
<PropDef name="renderError" type="({ message: string }) => React.Element">
  A React component which will be rendered in case of an error while rendering
  the chart. The component may accept the error `message` as its only prop.
</PropDef>

Additionally, the chart element also accepts all the [pointer event handlers](../interaction/1-pointer-events).
