...
<Chart width={300} height={100} view={[0, 0, 4, 5]} isCanvas={false}>
  <XAxis />
  <YAxis />
  <BarVerticalSeries
    data={[
...
The [`seriesStyles` addon](../addons/seriesStyles) can be used to generate `dataBoxLeftOffset` values.
...
<PropDef
  name="renderBar"
  type="({x: number, yMin: number, yMax: number, chartStyle }) => React.Element"
>
...
Additionally, the bar series component accepts all the [pointer event handlers](../interaction/1-pointer-events), which are passed on to the individual bars.
