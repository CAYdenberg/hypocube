...
To demonstrate how this works, we'll rebuild the [Tooltip example](/interaction/2-tooltips/) from the previous article to use the Voronoi addon instead. It should work in a similar way, except that every part of the chart area will map the nearest data point.
...
```javascript
import { useTooltip } from 'hypocube/build/js/addons/tooltips';
import { useVoronoi } from 'hypocube/build/js/addons/useVoronoi';
