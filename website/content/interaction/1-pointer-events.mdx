...
Check out this article to learn more about the [mental model behind pointer events](https://javascript.info/pointer-events). [MDN is the best place to get detailed info on when each event fill fire](https://developer.mozilla.org/en-US/docs/Web/API/Pointer_events). Finally, [the React docs](https://reactjs.org/docs/events.html#pointer-events) are always worth a look.

## Event data

Hypocube re-maps the argument passed to the handler so that the relevent coordinates on the Cartesian data scales are available. To see this, try clicking on the bars in the chart below - and have a look at your console.
...
Notice that the `elementPosition` corresponds to one of the pieces of data passed to the `BarSeries`. Similarly, the coordinates of `pointerPosition` correspond to the Cartesian scales of the chart.

In detail, the object passed to handlers has the following props:
