...
- `filterGestures: (gestureData: ChartGestureEvent) => boolean`. Used to ignore certain events. Since most users find mousewheel zoom annoying, the default is `gestureData => !gestureData.meta.includes('wheel')`
...
<PropDef name="next" type="Viewbox">
  Hypocube's "guess" as to the Viewbox that should be created by scrolled to by
  the gesture.
</PropDef>
<PropDef name="start" type="Viewbox">
  The original viewbox before the gesture.
</PropDef>
...
<PropDef name="meta" type="Array<'trackpad' | 'wheel' | 'pinch'>">
  An array of certain other data that can be gleaned from the state. The default
  `filterGesture` function uses this to ignore mouse wheel events, which most
  users find annoying.
</PropDef>
